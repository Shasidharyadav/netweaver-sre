...
def clamp_score(raw: float) -> float:
    """Clamp a score strictly between 0 and 1 (exclusive).
    
    The validator rejects scores of exactly 0.0 or 1.0.
    This ensures all scores are pure Python floats in (0.001, 0.999).
    """
...
class NetweaverSreEnvironment(Environment):
    ...
    def reset(self, **kwargs) -> NetweaverSreObservation:
        ...
        _GLOBAL_CACHE["active_task"] = forced if forced else kwargs.get(
        ...
        _GLOBAL_CACHE["logs"] = [
            f"CLUSTER INIT: Clos Topology Active. Running mode: {_GLOBAL_CACHE['active_task'].upper()}"
        ]
        ...
        if task == "easy":
            _GLOBAL_CACHE["logs"].append(
                f"ALERT: Node offline. Isolate {fnode} using DRAIN_TRAFFIC."
            )
        elif task == "medium":
            _GLOBAL_CACHE["queue_depths"]["spine_1"] = 99.9  # Buffer Congestion
            _GLOBAL_CACHE["logs"].append(
                f"WARN: Buffer overshoot on spine_1. Identify target threshold "
                f"(Target: {_GLOBAL_CACHE['target_pfc']}) and adjust via TUNE_PFC_THRESHOLD."
            )
        elif task == "hard":
            fault_idx = int(fnode.split("_")[1]) // 10  # Map node 0-99 to 10 sub-clusters
            _GLOBAL_CACHE["gradient_vars"][fault_idx] = 999.9  # Massive NaN variance
            _GLOBAL_CACHE["logs"].append(
                "CRITICAL: Loss diverging rapidly. Perform binary search triage using "
                "RUN_MINI_ITERATION (e.g., target='0-5') to locate NaN source then "
                "DRAIN_TRAFFIC the node."
            )
    ...
    def step(self, action: NetweaverSreAction) -> NetweaverSreObservation:  # type: ignore[override]
            ...
            if task == "medium":
                if val is not None:
                    ...
                    if distance <= 5.0:  # Agent tuned it close enough
                        ...
                        logs.append(
                            "INFO: Network partially relieved. Still experiencing collision."
                        )
                else:
            ...
            if task == "hard":
                try:
                    ...
                    if start <= fault_idx <= end:
                        if span == 0:
                            logs.append(
                                f"TRIAGE CONFIRMED: NaN source is {fnode} "
                                f"(sub-cluster {start}). Issue DRAIN_TRAFFIC "
                                f"with target='{fnode}'."
                            )
                        else:
                            mid = (start + end) // 2
                            logs.append(
                                f"TRIAGE HIT: Fault is inside range {start}-{end}. "
                                f"Narrow it — try {start}-{mid} or {mid+1}-{end} next."
                            )
                    else:
                        logs.append(
                            f"TRIAGE CLEAR: Range {start}-{end} is healthy. "
                            f"Search the complementary range (0-9 excluding this one)."
                        )
                except Exception:
                    logs.append(
                        "ERROR: RUN_MINI_ITERATION target must be format 'start-end' "
                        "(e.g. '0-5')."
                    )
            else:
                logs.append(
                    "ERROR: Reductive triage only effective for tracking SDC faults."
                )
        ...
        # Store the last grader score for the /grader endpoint
